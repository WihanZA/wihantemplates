...
# For more information about the YAML fields above
# See https://github.com/WihanZA/wihantemplates?tab=readme-ov-file#su_dissertation
...
When you click the **Knit** button (or `Ctrl + Shift + K`) a document will be generated based on the content of this document, as well as the underlying LaTeX template at `styles/template.tex`.
...
Default chunk options are set globally using `knitr::opts_chunk$set` with arguments such as  `echo`, `warning`, and `message`.^[For example, the code chunk option `echo` controls whether the code in an code chunk is printed in the final document. This is set globally to `echo = F` in the `setup` chunk.However, in an individual code chunk, like `cars`, you can specify `echo = T` if you want its code printed.]
...
head(mtcars)[,1:4] %>% 
  ...
  footnote(general_title = "My footnote:",
